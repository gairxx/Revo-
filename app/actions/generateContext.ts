...
export async function generateVehicleContextAction(
  vehicle: Omit<Vehicle, "id" | "contextString" | "createdAt">,
): Promise<string> {
  const API_KEY = process.env.GEMINI_API_KEY || process.env.API_KEY

  if (!API_KEY) {
    throw new Error("API Key missing - GEMINI_API_KEY not configured")
  }
